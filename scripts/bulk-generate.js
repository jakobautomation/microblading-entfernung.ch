#!/usr/bin/env node
/**
 * Bulk Blog Generator für microblading-entfernung.ch
 * Generiert 30 Artikel mit gestaffelten Publish-Dates
 * Aufruf: SUPABASE_SERVICE_KEY=... ANTHROPIC_API_KEY=... node bulk-generate.js
 */
...
// 30 neue Themen (die 15 aus generate-blog.js sind bereits erzeugt oder reserviert)
const BULK_TOPICS = [
  ...
  { slug: 'microblading-entfernung-schwangerschaft', title: 'Microblading entfernen während oder nach der Schwangerschaft — was du wissen musst', keywords: 'microblading entfernung schwangerschaft stillzeit' },
  { slug: 'microblading-haarstrich-technik-entfernen', title: 'Haarstrich-Technik Microblading entfernen — wie komplex ist es wirklich?', keywords: 'haarstrich microblading entfernen laser schweiz' },
  { slug: 'powder-brows-entfernen', title: 'Powder Brows entfernen — Unterschied zu klassischem Microblading', keywords: 'powder brows entfernen laser schweiz' },
  { slug: 'ombre-brows-entfernen', title: 'Ombré Brows entfernen mit PicoSure — alles was du wissen musst', keywords: 'ombre brows entfernen schweiz laser' },
  { slug: 'microblading-sitzungen-wie-viele', title: 'Wie viele Sitzungen braucht man für die Microblading-Entfernung?', keywords: 'microblading entfernen sitzungen anzahl schweiz' },
  { slug: 'microblading-haut-sensibel-entfernen', title: 'Sensible Haut und Microblading-Entfernung — worauf du achten musst', keywords: 'sensible haut microblading laser entfernung' },
  { slug: 'microblading-entfernung-winterthur', title: 'Microblading entfernen Winterthur — Warum Kreuzlingen die bessere Wahl ist', keywords: 'microblading entfernen winterthur vergleich kreuzlingen' },
  { slug: 'microblading-entfernung-st-gallen', title: 'Microblading entfernen St. Gallen — Anfahrt und Optionen', keywords: 'microblading entfernen st gallen ostschweiz' },
  { slug: 'tattoo-vs-microblading-entfernen', title: 'Tattoo vs. Microblading entfernen — was ist einfacher?', keywords: 'tattoo microblading vergleich entfernen laser' },
  { slug: 'microblading-rot-verfaerbt-entfernen', title: 'Microblading mit Rotstich entfernen — was den Farbton schwierig macht', keywords: 'microblading rotstich entfernen laser pigment' },
  { slug: 'microblading-lip-liner-entfernen', title: 'Permanenter Lip-Liner entfernen — PicoSure auch für Lippen?', keywords: 'permanenter lip liner entfernen laser schweiz' },
  { slug: 'microblading-entfernung-kosten-krankenkasse', title: 'Zahlt die Krankenkasse Microblading-Entfernung? — Ehrliche Antwort', keywords: 'krankenkasse microblading entfernung kosten schweiz' },
  { slug: 'pmu-entfernung-augenlid', title: 'Eyeliner Permanent Make-Up entfernen — Risiken und Möglichkeiten', keywords: 'permanent eyeliner entfernen augenlid laser schweiz' },
  { slug: 'microblading-entfernung-fruehjahr-timing', title: 'Wann ist der beste Zeitpunkt für Microblading-Entfernung? Sommer vs. Winter', keywords: 'microblading entfernung wann sommer winter timing' },
  { slug: 'microblading-entfernung-konstanz-deutschland', title: 'Microblading entfernen Konstanz — Patienten aus Deutschland in Kreuzlingen', keywords: 'microblading entfernen konstanz deutschland kreuzlingen grenze' },
];
...
async function updateSitemap(slug, date) {
  ...
  if (sitemap.includes(`/blog/${slug}/`)) return; // Already in sitemap
  const newEntry = `  <url>
...
async function generateArticle(topic, anthropic, publishDate) {
...
async function main() {
  ...
  const { data: published } = await supabase.from('blog_articles').select('slug');
  const publishedSlugs = new Set((published || []).map(a => a.slug));

  const toGenerate = BULK_TOPICS.filter(t => !publishedSlugs.has(t.slug));
  console.log(`${toGenerate.length} Artikel zu generieren (${publishedSlugs.size} bereits vorhanden)`);

  // Startdatum: 27. März 2026 (die 6 vorhandenen haben 21.-26. März)
  const startDate = new Date('2026-03-27T10:00:00.000Z');
  let dayOffset = 0;
  ...
  for (const topic of toGenerate) {
    ...
    console.log(`[${dayOffset + 1}/${toGenerate.length}] Generiere: ${topic.title} (${dateStr})`);

    try {
      const article = await generateArticle(topic, anthropic, publishDate.toISOString());

      // Save to Supabase
      const { error } = await supabase.from('blog_articles').insert([article]);
      if (error) {
        console.error(`  ❌ Supabase error für ${topic.slug}:`, error.message);
        dayOffset++;
        continue;
      }
      ...
      // Update sitemap
      await updateSitemap(article.slug, dateStr);

      console.log(`  ✅ /blog/${article.slug}/ (${dateStr})`);
    } catch (err) {
  ...
  console.log('\n✅ Bulk-Generierung abgeschlossen.');
}
